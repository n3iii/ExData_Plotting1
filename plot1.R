## This script will plot the frequency of the Global Active Power over the 
...
wrkDF <- data.frame(apwr=targetDF$apwr, 
                   stringsAsFactors = FALSE)

## Create the histogram for plot 1 and save it to a png file using default 480x480
png("plot1.png")
hist(wrkDF$apwr, # use the power column of the working dataframe as the x axis
     main = "Global Active Power", #give it the title
     xlab="Global Active Power (killowatts)", #create x axis label
     col="red") #make the histogram red
dev.off()
